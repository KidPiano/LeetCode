...
2. Use a modified version of [DFS](https://kidpiano.github.io/Algorithms/Graph/DFS.html){:target="_blank"} to find a reverse [topological sort](){:target="_blank"} of the graph if it is acyclic.
...
<pre style="font-size:0.8rem; background-color:whitesmoke"><code class="prettyprint" style="font-weight:bold">// returns true if the graph is acyclic
boolean DFS() {
...
<pre style="font-size:0.8rem; background-color:whitesmoke"><code class="prettyprint" style="font-weight:bold">// returns a reverse topological sort
<mark>int[]</mark> DFS() {
    for (int u = 0; u < numVertices; u++)
        if (color[u] == WHITE && visit(u))
...
<iframe src="https://leetcode.com/playground/NikxG5ut/shared" frameBorder="0" width="100%" height="835"></iframe>
